...
<div id="layoutSidenav_content" style="background-color: rgb(240,236,236);">
    <main>
        <div class="container-fluid px-4">
            <div class="d">
                <h1 class="mt-4">
                    <a href="{{ route('announcements.indexpage') }}" style="color: black; text-decoration:none;">Announcement</a> | <span style="font-size:22px;">Edit Announcement</span>
                </h1>
            </div>
                ...
                <div class="content-wrap container mb-2">
                    <div class="form-wrap p-2">
                        <div class="table-wrap d-flex justify-content-center">
                            <form class="container" id="announcement-form" method="POST" enctype="multipart/form-data" action="{{ route('announcements.update', $announcement->id) }}">
                                @csrf
                                
                                <div class="d-lg-flex d-md-flex container">
                                    <div class="text-center">
                                        <img class="rounded w-100" id="prev-img-event" height="250px" src="{{ $announcement->cover ? asset('announcement_img/' . base64_decode($announcement->cover)) : asset('assets/imgs_uploads/bg-img2.jpg') }}">
                                    ...
                                    <div class="m-1 container">
                                        <div class="input-group mb-1">
                                            ...
                                            <input class="form-control" aria-label="Title" aria-describedby="basic-title" type="text" name="event-title" placeholder="My title.." autocomplete="off" value="{{ old('event-title', base64_decode($announcement->title)) }}">
                                        </div>
                                        @error('event-title')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                        <textarea id="markdown-editor-container" style="min-height:300px;" class="w-100 form-control mt-1" name="event-content" autocomplete="off" placeholder="Content..">{{ old('event-content', base64_decode($announcement->content)) }}</textarea>
                                        @error('event-content')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                        <input class="w-50 btn btn-outline-success mt-2" type="submit" value="Update Announcement">
                                    </div>
                                </div>
                            </form>
                        </div>
