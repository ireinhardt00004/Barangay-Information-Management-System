...
?>
    <div class="row">
        <div class="col-md-12 text-center" style="background-color:rgb(255,255,255,0.4); padding:10px; border-radius:10px;">
            ...
            <!-- 3 -->
            <span style="background-color:rgb(0,0,0,0.1);" class="btn mx-3 mt-1"><b>Pending</b> = <button class="btn btn-sm btn-outline-warning"><i class="fa-regular fa-clock"></i></button></span>
            <!-- 4 -->
            <span style="background-color:rgb(0,0,0,0.1);" class="btn mx-3 mt-1"><b>Approve</b> = <button class="btn btn-sm btn-outline-success"><i class="fa-solid fa-check"></i></button></span>
        </div>
    </div>
    <br>
...
foreach($navs_data as $data){
    ...
    $decline_btn = '<button onclick="decline(\''.$id.'\')" class="mx-1 btn btn-sm btn-outline-danger"><i class="fa-solid fa-x"></i></button>';
    ...
    $btns = '<div id="rowz'.$id.'" class="d-flex">'.$view_btn.$decline_btn.$pending_btn.$approve_btn.'</div>';
    $endz = ',';
    if($data == $last_data){
        $endz = '';
    }
...
function decline(service_id){
    Swal.fire({
        ...
        <form class="" method="post" enctype="multipart/form-data">
            <input name="s_id" hidden value="`+service_id+`">
            <div class="container">
                <textarea class="form-control" placeholder="Your request is declined because..." name="reason"></textarea>
            </div>
            <input class="btn btn-danger m-3" type="submit" name="decline" value="Decline">
