...
if(isset($_POST['edit-event'])){
    ...
    if(empty($cover)){
        $e_data = data_extract($id, 'id', get_all_announcement());
        $cover = base64_decode($e_data['cover']);
    }

    $cover = base64_encode($cover);
    $conn = $GLOBALS['conn'];
    $sql = $conn->prepare("UPDATE `announcement` SET cover=:cover, title=:title, content=:content WHERE id=:id;");
    $sql->bindParam(":id",$id);
    $sql->bindParam(":cover",$cover);
    $sql->bindParam(":title",$title);
    $sql->bindParam(":content",$content);
    $sql->execute();
    act_logger("Edited announcement | ID: '$id' ", $current_uri);
