...
foreach($navs_data as $data){
    ...
    $date_approve = $data['date_modified'];
    // $decline_btn = '<button onclick="mod_request(2,\''.$id.'\')" class="mx-1 btn btn-sm btn-outline-danger"><i class="fa-solid fa-x"></i></button>';
    ...
    $btns = '<div id="rowz'.$id.'" class="d-flex">'.$view_btn.$decline_btn.$pending_btn.'</div>';
    $endz = ',';
    if($data == $last_data){
        $endz = '';
    }
...
?>
        ...
        var table = new Tabulator("#table-container", {
            ...
            columns: [
                ...
                { title: "Date Approved", field: "date_approved", minWidth: 200 },
                { title: "Status", field: "status", minWidth: 120 },
                { title: "Action", field: "Action", formatter: "html", minWidth: 150},
            ],
        });
